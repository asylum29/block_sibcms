    ...
    public function render_category_statistic_table(category_statistic_table $widget) {
        ...
        if (count($widget->categories) > 0) {
            ...
            foreach ($widget->categories as $category) {
                ...
                $coursesstr = $category->courses_total . ' ('. \html_writer::tag('a', get_string('key9', 'block_sibcms'),
                        array('href' => new \moodle_url('/blocks/sibcms/courses.php', array('category' => $category->id)))) . ')';
    ...
    public function render_category_courses_table(category_courses_table $widget) {
        ...
        if (count($widget->courses) > 0) {
            ...
            foreach ($widget->courses as $course) {
                ...
                $table->data[] = array(
                    \block_sibcms\sibcms_api::need_attention($course->id) ? \html_writer::span('!', 'bold red text-center') : '',
                    $course->fullname,
                    $time_ago,
                    $status,
                    \html_writer::tag('a', get_string('key19', 'block_sibcms'),
                        array(
                            'href' => new \moodle_url('/blocks/sibcms/course.php', array('id' => $course->id, 'category' => $widget->categoryid)),
                            'class' => 'btn'
                        )
                    )
                );           
            ...
            $result .= \html_writer::table($table);
            $result .= $OUTPUT->paging_bar($widget->coursescount, $widget->page, 20, new \moodle_url('/blocks/sibcms/courses.php', array('category' => $widget->categoryid)));
        } else {
            $result .= $OUTPUT->heading(get_string('key6', 'block_sibcms'));
        }
        return $result;
